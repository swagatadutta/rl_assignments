...
class CEMAgent(base_agent.BaseAgent):
    ...
    def _init_train(self):
        ...
        self._param_mean = torch.nn.utils.parameters_to_vector(self._model.parameters()).to(self._device)
        self._param_std = 0.5 * torch.ones_like(self._param_mean).to(self._device)
        self._best_return = None
        self._best_params = None
        return
    ...
    def _build_model(self, config):
        ...
        self._model = cem_model.CEMModel(model_config, self._env).to(self._device)
        return
    ...
    def _decide_action(self, obs, info):
        ...
        norm_a = norm_action_dist.mode.detach()
    ...
    def _sample_candidates(self, n):
        ...
        candidates = torch.normal(mean=self._param_mean, std=self._param_std.expand(n, param_size))

        return candidates.to(self._device)
    ...
    def _eval_candidates(self, candidates):
        ...
        for i in range(n):
            # Copy candidate's parameters to the model
            torch.nn.utils.vector_to_parameters(candidates[i].clone().detach(), self._model.parameters())

            # Evaluate the performance of the set of parameters
            rollout_test = self._rollout_test(num_episodes=self._eps_per_candidate)
            ret, ep_len = rollout_test['mean_return'], rollout_test['mean_ep_len']

            # Record the average return and average episode length
            rets[i] = ret
            ep_lens[i] = ep_len
    ...
    def _compute_new_params(self, params, rets):
        ...
        '''
        '''
        weights = torch.from_numpy(rets - rets.mean()).to(self._device)

        new_mean = (weights[:, None] * params).sum(dim=0) / weights.sum()
        new_std = torch.sqrt(((weights[:, None] * (params - new_mean) ** 2).sum(dim=0) / weights.sum()) + self._min_param_std)
        print(new_mean, new_std)
        return new_mean, new_std
        '''
        param_size = self._param_mean.shape[0]

        # Convert rets to a PyTorch tensor
        rets_tensor = torch.tensor(rets, dtype=torch.float32)

        # Number of elite samples determined by self.elite_ratio
        num_elite = int(self._elite_ratio * params.size(0))

        # Sort returns in descending order and get indices of elite samples
        elite_indices = torch.argsort(rets_tensor, descending=True)[:num_elite]

        # Select elite samples using indices
        elite_params = params[elite_indices]

        # Compute new mean and std by fitting Gaussian to elite samples
        new_mean = torch.mean(elite_params, dim=0)

        # Compute variance with a minimum value to avoid too small std deviation
        new_var = torch.var(elite_params, dim=0, unbiased=False) + self._min_param_std**2

        # Compute standard deviation from variance
        new_std = torch.sqrt(new_var)

        # Clamp standard deviation to be at least self.min_param_std
        new_std = torch.clamp(new_std, min=self._min_param_std)

        return new_mean.to(self._device), new_std.to(self._device)