...
class PGAgent(base_agent.BaseAgent):
    ...
    def _calc_adv(self, norm_obs, ret):
        ...
        V_t = self._model.eval_critic(norm_obs).squeeze()   
        adv = ret - V_t
    ...
    def _calc_critic_loss(self, norm_obs, tar_val):
        ...
        predicted_val = self._model.eval_critic(norm_obs).squeeze()  
        loss = torch.nn.functional.mse_loss(predicted_val, tar_val)
        return loss
    ...
    def _calc_actor_loss(self, norm_obs, norm_a, adv):
        ...
        log_probs = self._model.eval_actor(norm_obs).log_prob(norm_a)
        loss = -torch.mean(log_probs * adv.detach())
        return loss