...
class BCAgent(base_agent.BaseAgent):
    ...
    def _compute_actor_loss(self, norm_obs, norm_expert_a):
        ...
        '''
        # placeholder
        # Use the model to evaluate the policy's action distribution
        norm_pred_a_dist = self._model.eval_actor(norm_obs)

        # Compute the negative log likelihood of the expert actions under the policy
        loss = -norm_pred_a_dist.log_prob(norm_expert_a).mean()
